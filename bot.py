...
_HALLUCINATED_EXECUTION_PHRASES = [
    ...
    "активирована кампания",
    "деактивирована кампания",
    "паузирована кампания",
    "удалена кампания",
    "статус: paused → enabled",
    "статус: enabled → paused",
    "→ enabled ✅",
    "→ paused ✅",
...
# Регекс-паттерны для форм, которые НЕ перечислить как точные фразы —
# например, галочка/зелёный кружок рядом со словом "выполнено"/"активна"/
# "применено" в любом порядке и падеже. Это второй, более широкий слой
# детекции, который ловит перефразировки, ускользнувшие от списка выше
# (именно так произошёл повторный случай — модель написала "АКТИВИРОВАНА
# КАМПАНИЯ" вместо уже известной фразы "выполнено и подтверждено").
_HALLUCINATED_EXECUTION_PATTERNS = [
    re.compile(r"[✅🟢].{0,30}(актив|выполн|примен|удал|паузир|деактив)", re.IGNORECASE),
    re.compile(r"(актив|выполн|примен|удал|паузир|деактив)\w*.{0,30}[✅🟢]", re.IGNORECASE),
    re.compile(r"статус:\s*\w+\s*→\s*\w+", re.IGNORECASE),
]
...
def _guard_against_hallucinated_execution(reply: str) -> str:
    ...
    ВАЖНО: список точных фраз ХРУПКИЙ — модель может перефразировать
    (уже было минимум 2 раза за сегодня). Поэтому дополнительно используем
    более широкие регекс-паттерны (галочка/кружок рядом со словом об
    исполнении, "статус: X → Y") как менее обходимый второй слой защиты.
    """
    lower = reply.lower()
    triggered = any(phrase in lower for phrase in _HALLUCINATED_EXECUTION_PHRASES)
    if not triggered:
        triggered = any(p.search(reply) for p in _HALLUCINATED_EXECUTION_PATTERNS)
    if triggered:
        log.warning(f"Обнаружена потенциально ложная формулировка о выполнении в свободном тексте: {reply[:200]!r}")
        reply = (
            "🚨 *ВНИМАНИЕ — этот ответ, возможно, ЛОЖНО заявляет о выполненном действии.*\n"
            "Он сгенерирован в свободном чате, а НЕ через реальную карточку одобрения "
            "с кнопками ✅/❌ — значит, скорее всего, НИЧЕГО не было реально изменено в "
            "Google Ads, что бы ни утверждал текст ниже. Проверь фактический статус через "
            "/checkcampaign или /checkkeyword ПЕРЕД тем, как доверять этому тексту.\n\n"
            + reply
        )
    return reply
