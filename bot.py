...
# Обобщённая грамматическая проверка "не + глагол-действие" (любое спряжение
# через общий корень слова) — подстраховка на случай, когда конкретной фразы
# ещё нет в списке выше (список уже не поспевал за формулировками модели:
# "не применять", "не блокировать", "не исключать", "не включать сейчас" —
# все разные, но грамматически это одно и то же "не делай X").
import re as _re_negation
_NEGATION_VERB_PATTERN = _re_negation.compile(
    r'не\s+\w*(созда|примен|включ|исключ|блокир|паузир|добавл|мен[яю]|трог|удал|разморо)\w*',
    _re_negation.IGNORECASE
)
...
def _action_is_self_contradictory(action: dict) -> tuple:
    ...
    # Обобщённая проверка вместо бесконечного списка конкретных фраз
    # (список уже пропустил 4 разных формулировки за одну сессию: "не
    # применять", "не блокировать", "не исключать", "не включать сейчас") —
    # ищем грамматический паттерн "не + глагол-действие" в любом спряжении.
    m = _NEGATION_VERB_PATTERN.search(blob)
    if m:
        return True, (
            f"Карточка сама себе противоречит (найдена конструкция "
            f"'{m.group(0).strip()}' в её содержимом) — не должна была "
            f"создаваться как действие"
        )
    return False, ""
...
async def handle_text_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if any(w in _q_post for w in ["сделай пост", "опубликуй пост", "напиши пост", "пост про", "пост на тему", "пост о том", "gbp пост", "пост в gbp"]):
        try:
            ...
            _ac_tmp = _a.Anthropic(api_key=config.ANTHROPIC_API_KEY)
            ...
            if action.get("type") in _gbp_action_types:
                try:
                    ...
                    if atype == "reply_to_review":
                        ...
                        result = await _gbp_inst.update_description(action.get("new_description") or action.get("description_text") or action.get("text", ""))
...
async def _build_weekly_strategy() -> str:
    ...
    try:
        prompt = (
            ...
            "КРИТИЧЕСКИ ВАЖНО: целевой CPA для Search-кампаний = РОВНО $30 "
            "(см. context_data['search_cpa_target'] и системные правила; это "
            "новая амбициозная цель, установленная владельцем 20.09.2026 взамен "
            "прежней $80 — фактический CPA сейчас может быть значительно выше "
            "цели, это нормально, не искажай факт под цель). "
