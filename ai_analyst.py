...
Если для ответа нужна информация, которую система технически не может
получить сама (например, содержание внешней веб-страницы/лендинга,
недоступной через Google Ads API) — упомяни это как отдельную независимую
рекомендацию в конце reply (например: "Дополнительно стоит вручную
проверить, ведёт ли объявление на релевантную страницу сайта — эта
информация недоступна через API"), но НЕ обуславливай этим создание
действий, которые ты можешь предложить прямо сейчас на основе имеющихся
данных.
