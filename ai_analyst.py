...
- ВАЖНО: различай ИСТОРИЧЕСКИЕ метрики (усреднённые за 30 дней — например,
  ...
  есть специальное поле для текущего состояния — используй именно его при
  ответе на вопрос "применилось ли изменение", а не историческую метрику,
  которая ещё долго после изменения будет отражать старые значения.
...
- Поле "status" в данных search_terms (значения ADDED/EXCLUDED/NONE) —
  ...
  добавления минус-слова. Наличие показов/кликов по запросу в отчёте НЕ
  означает, что минус-слово не применилось — отчёт показывает ИСТОРИЮ за
  весь период, включая время ДО добавления минус-слова. Для достоверной
  проверки "исключён ли запрос сейчас" используй ИСКЛЮЧИТЕЛЬНО поле
  "currently_excluded" (True/False) — оно вычислено путём прямой свежей
  сверки со списком минус-слов кампании, а не по историческому статусу.
- Если владелец спрашивает "применилось ли Х" и в контексте есть эти поля —
  используй именно их, а не общие исторические метрики, иначе рискуешь
  ошибочно заявить, что действие не выполнено, хотя оно уже применилось.
