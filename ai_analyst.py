...
3. days — ТОЛЬКО если action_type="audit_lsa_calls": целое число дней,
   которое нужно передать в аудит, исходя из формулировки владельца.
   Примеры: "за неделю"/"за прошедшую неделю" → 7, "за месяц"/"за июнь"/
   "за прошлый месяц" → 30, "за квартал" → 90, "за 2 недели" → 14.
   Если не указано явно и не следует из контекста — используй 7.
   Если action_type не audit_lsa_calls — верни 0.
4. data_needed — СПИСОК типов данных, нужных для полного ответа (можно
   ...
   search_terms (поисковые запросы для минус-слов), lsa_leads (детальные
   данные по лидам Local Services Ads — категория услуги, статус, кредиты —
   используй для вопросов про качество/релевантность LSA-лидов, спорные
   списания, категории услуг не по профилю, БЕЗ прослушивания звонков),
   seasonal (сезонные рекомендации + бюджеты). Если action_type="audit_lsa_calls"
   — верни пустой список [] (данные соберутся отдельно). Если вопрос общий
   и данные не нужны — тоже верни пустой список [].
...
Верни ТОЛЬКО JSON:
{{"intent": "chat|action", "action_type": "...", "days": 0, "data_needed": ["..."], "account": "ads|lsa|both"}}
"""
        result = await self._call_claude(prompt, max_tokens=300, history=history)
        if "_error" in result:
            return {"intent": "chat", "action_type": "none", "days": 0, "data_needed": ["campaigns"], "account": "both"}
...
"""
    ...
    async def analyze_lsa_call(self, transcript: str, lead_metadata: dict) -> dict:
        """
        Анализирует транскрипт звонка по LSA-лиду и определяет, соответствует
        ли запрос клиента услугам бизнеса (appliance repair). Рекомендует
        оспаривание, только если явно видно несоответствие категории —
        не выдумывает несоответствие, если из транскрипта не ясно.
        """
        prompt = f"""
