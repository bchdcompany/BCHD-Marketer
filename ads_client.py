...
class GoogleAdsClient:
    ...
    def __init__(self, config):
        self.config = config
        self.customer_id = config.GOOGLE_ADS_CUSTOMER_ID        # 936
        self.lsa_customer_id = config.GOOGLE_ADS_LSA_CUSTOMER_ID  # 667
        self._client = None
    ...
    def _query(self, customer_id: str, query: str) -> list:
        """Универсальный метод запроса для любого аккаунта"""
        client = self._get_client()
        ga_service = client.get_service("GoogleAdsService")
        response = ga_service.search(customer_id=customer_id, query=query)
        return list(response)

    # ── ПОЛУЧЕНИЕ ДАННЫХ (оба аккаунта) ─────────────────

    async def get_both_accounts_summary(self) -> dict:
        """Сводка по обоим аккаунтам за 30 дней"""
    ...
    def get_current_season_recommendations(self) -> dict:
        """Сезонные рекомендации по ключевым словам"""
        month = datetime.now().month

        if month in [12, 1, 2]:
            return {
                'season': 'winter', 'season_name': 'Зима',
                'boost_keywords': ['refrigerator repair', 'fridge repair', 'freezer repair', 'washer repair', 'dryer repair', 'dishwasher repair'],
                'reduce_keywords': ['air conditioner repair', 'AC repair', 'HVAC repair'],
                'boost_pct': 20, 'reduce_pct': -30,
                'reason': 'Зима: высокий спрос на ремонт холодильников и стиральных машин. Кондиционеры — низкий сезон.',
                'ad_copy_tip': 'Добавь: "Same-day service", "Available today", "Emergency repair".',
            }
        elif month in [3, 4, 5]:
            return {
                'season': 'spring', 'season_name': 'Весна',
                'boost_keywords': ['appliance repair', 'refrigerator repair', 'washer repair', 'dryer repair', 'oven repair', 'stove repair'],
                'reduce_keywords': [],
                'boost_pct': 10, 'reduce_pct': 0,
                'reason': 'Весна: равномерный спрос. Люди делают весеннюю уборку и замечают проблемы с техникой.',
                'ad_copy_tip': 'Акцент на "Spring special offer", "Free estimate".',
            }
        elif month in [6, 7, 8]:
            return {
                'season': 'summer', 'season_name': 'Лето',
                'boost_keywords': ['air conditioner repair', 'AC repair', 'HVAC repair', 'refrigerator repair', 'fridge not cooling', 'freezer repair', 'ice maker repair'],
                'reduce_keywords': ['dryer repair', 'washer repair'],
                'boost_pct': 30, 'reduce_pct': -15,
                'reason': 'Лето: пиковый сезон для ремонта кондиционеров и холодильников.',
                'ad_copy_tip': '"Same-day AC repair", "24/7 emergency". Люди не могут ждать когда жарко.',
            }
        else:
            return {
                'season': 'fall', 'season_name': 'Осень',
                'boost_keywords': ['oven repair', 'stove repair', 'range repair', 'dishwasher repair', 'refrigerator repair', 'washer repair', 'dryer repair'],
                'reduce_keywords': ['air conditioner repair', 'AC repair'],
                'boost_pct': 15, 'reduce_pct': -25,
                'reason': 'Осень: спрос на ремонт духовок и плит растёт перед праздниками.',
                'ad_copy_tip': '"Ready for the holidays?", "Don\'t let a broken appliance ruin Thanksgiving".',
            }

    async def execute_action(self, action: dict) -> dict:
        """Точка входа для всех изменений. Вызывается ТОЛЬКО после одобрения владельца."""
        action_type = action.get('type')
        account = action.get('account', 'ads')
        customer_id = self.lsa_customer_id if account == "lsa" else self.customer_id
        log.info(f"Выполняю одобренное действие: {action_type} для аккаунта {account} ({customer_id})")
    ...
    async def _change_budget(self, action: dict, customer_id: str = None) -> dict:
        ...
        campaign_budget_service.mutate_campaign_budgets(customer_id=customer_id, operations=[op])
    ...
    async def _update_bid(self, action: dict, customer_id: str = None) -> dict:
        ...
        ad_group_criterion_service.mutate_ad_group_criteria(customer_id=customer_id, operations=[op])
    ...
    async def _pause_campaign(self, action: dict, customer_id: str = None) -> dict:
        ...
        campaign = op.update
        campaign.resource_name = f"customers/{customer_id}/campaigns/{action['campaign_id']}"
    ...
    async def _apply_seasonal_adjustments(self, action: dict, customer_id: str = None) -> dict:
        ...
        for adj in adjustments:
            try:
                ...
                budget = op2.update
                budget.resource_name = f"customers/{customer_id}/campaignBudgets/{adj['budget_id']}"
                budget.amount_micros = int(new_budget * 1_000_000)
                op2.update_mask.paths.append("amount_micros")
                budget_service = client.get_service("CampaignBudgetService")
                budget_service.mutate_campaign_budgets(customer_id=customer_id, operations=[op2])
                applied += 1
            except Exception as e:
