...
class ReportGenerator:
    ...
    def format_auction_report(self, data: dict, analysis: dict) -> str:
        ...
        lines = [
            ...
            f"📅 Период: {data.get('date_from')} — {data.get('date_to')}",
            f"",
            f"{pos_e} *Позиция: {position.upper()}*",
    ...
    def format_ab_test_report(self, data: dict, analysis: dict) -> str:
        ...
        if ab_results:
            ...
            for result in ab_results:
                confidence_e = {"high": "🟢", "medium": "🟡", "low": "🔴"}.get(
    ...
    def format_seasonal_report(self, season_data: dict, action_plan: dict) -> str:
        ...
        lines = [
            ...
            f"{'─' * 28}",
            f"",
            f"📌 *Обоснование:*\n{season_data.get('reason', '')}",
            f"",
            f"💡 *Совет по текстам:*\n_{season_data.get('ad_copy_tip', '')}_",
            f"",
        ]
        ...
        if adjustments:
            ...
            if boost:
                ...
                for adj in boost:
                    ...
                    lines.append(
                        f"• *{adj['campaign_name']}*\n"
                        f"  ${adj['current_budget']:.2f} → ${new_budget:.2f}/день\n"
                        f"  _{adj['reason']}_"
                    )
                lines.append("")
